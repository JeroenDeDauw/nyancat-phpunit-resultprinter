...
class FixedScoreboard
{
    ...
    /**
     * @param Cat $cat             The animated ASCII cat.
     * @param Rainbow $rainbow     The animated ASCII rainbow.
     ...
     */
    protected function drawScoreboard($percent)
    {
        $size = count($this->teams) + 1;
        $this->preamble($size);
        ...
        foreach ($this->teams as $key => $team) {
            $this->write(sprintf(
                ...
                self::ESC,
                $team->getColor(),
                $team->getScore(), // returns int, the doc is lying
                self::NND
            ));
        }
